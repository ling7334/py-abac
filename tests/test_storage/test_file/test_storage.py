...
pytestmark = [pytest.mark.file, pytest.mark.integration]
...
def test_add(st):
    policy_json = {
        ...
        "targets": {"subject_id": ["abc", "a*"], "resource_id": ["123"], "action_id": "*"},
        "effect": "deny",
        "priority": 0,
    }
...
@pytest.mark.parametrize(
    "request_json, num",
    [
        ({"subject": {"id": "a"}, "resource": {"id": str(uuid.uuid4())}, "action": {"id": str(uuid.uuid4())}}, 4),
        ({"subject": {"id": "ab"}, "resource": {"id": str(uuid.uuid4())}, "action": {"id": str(uuid.uuid4())}}, 4),
        ({"subject": {"id": "abc"}, "resource": {"id": str(uuid.uuid4())}, "action": {"id": str(uuid.uuid4())}}, 4),
        ({"subject": {"id": "acb"}, "resource": {"id": str(uuid.uuid4())}, "action": {"id": str(uuid.uuid4())}}, 4),
        ({"subject": {"id": "axc"}, "resource": {"id": str(uuid.uuid4())}, "action": {"id": str(uuid.uuid4())}}, 4),
    ],
)
def test_find_for_target(st, request_json, num):
    st.add(Policy.from_json({"uid": "1", "rules": {}, "targets": {}, "effect": "deny"}))
    st.add(Policy.from_json({"uid": "2", "rules": {}, "targets": {"subject_id": "ab*"}, "effect": "deny"}))
    st.add(Policy.from_json({"uid": "3", "rules": {}, "targets": {"subject_id": "a*b"}, "effect": "deny"}))
...
def test_update(st):
    ...
    p = Policy.from_json({"uid": "2", "description": "foo", "rules": {}, "targets": {}, "effect": "deny"})
    st.add(p)
    assert "2" == st.get("2").uid
    p = Policy.from_json(
        {
